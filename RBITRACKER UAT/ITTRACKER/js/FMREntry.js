...
function FRLoad() {
    ...
    if (acc == '1') { var flag = "getfraudDept"; }
    else if (acc == '0') { var flag = "getfraudIndivi"; }
    var data = $("[id*=hdDept]").val() + '~' + usr;
    $.ajax({
        type: "post",
        contentType: "application/json; charset=utf-8",
        url: "FMR1.aspx/getFillData",
...
function empdata() {
    ...
    $.ajax({
        ...
        type: "post",
        contentType: "application/json; charset=utf-8",
        url: "FMR1.aspx/LoadFraudDetails",
...
function FRdodata() {
   ...
   // alert(FraudType);
    $.ajax({
        ...
        type: "post",
        contentType: "application/json; charset=utf-8",
        url: "FMR1.aspx/LoadDoDetails",
        data: "{typ:'Branch2', val1 :' " + FraudType + "'}",
        dataType: "json",
        success: function (Result) {
            Result = Result.d;
...
function fillFRData() {
    ...
    if (acc == '0') {
        ...
        document.getElementById("AUDIT").style.pointerEvents = 'none';
        document.getElementById("RIIM").style.pointerEvents = 'none';
        document.getElementById("COMP").style.pointerEvents = 'none';
        //document.getElementById("AUDIT").style.pointerEvents = "none";
        document.getElementById("FrNo").style.pointerEvents = 'none';
        document.getElementById("CompRemarks").style.pointerEvents = 'none';
    }
    else if (dept == '4' || dept == '23') {

        document.getElementById("FMR1A").style.pointerEvents = 'none';
        document.getElementById("FMR1B").style.pointerEvents = 'none';
        document.getElementById("RIIM").style.pointerEvents = 'none';
        document.getElementById("COMP").style.pointerEvents = 'none';
        document.getElementById("CompRemarks").style.pointerEvents = 'none';
    }
    ...
    if (acc == '0' || dept == '4' || dept == '598' || dept == '23' || dept == '547') {
        ...
        $.ajax({
            ...
            success: function (Result) {
                ...
                var resdata43 = res1.split("~")[42].toString(); var resdata44 = res1.split("~")[43].toString();
    ...
    if (acc == '0' ||dept == '4' || dept == '598' || dept == '23' || dept == '547') {
        ...
        $('#txt_insrsn').val(''); $('#txt_riimtotamt').val(''); $('#txt_ins').val(''); $('#txt_othsrc').val(''); $('#txt_losstot').val('');
    ...
    $.ajax({
        ...
        type: "post",
        contentType: "application/json; charset=utf-8",
        url: "FMR1.aspx/checkcompdata",
        data: "{typ:'checkcompdata', val1 :' " + FraudType + "'}",
        dataType: "json",
        success: function (Result) {
            var res1 = Result.d;
...
function loadInsuranceClaim() {
    ...
    $.ajax({
        ...
        type: "post",
        contentType: "application/json; charset=utf-8",
        url: "FMR1.aspx/ddrInsuranceClaim",
...
function filltab(data) {
    ...
    if ($("#tblObject tr").length == 0) {
        $("#tblObject").empty();
        $('#tblObject').append('<tr style="background-color:darkgrey;color:black"><th class="text-center">empcode</th><th class="text-center">empname</th><th class="text-center">Designation</th><th class="text-center">Whether suspended /Dt.of Suspension</th><th class= style="length:5px">Date of issue</th><th class="text-center">Date of commencement</th><th class="text-center">Date of completion</th><th class="text-center">Date of issue of final orders</th><th class="text-center">Punishment awarded</th><th class="text-center">Details of prosecution</th><th class="text-center">DELETE</th></tr>');
    }
    //var sno = $('#tableData tr').length;
    for (i = 0; i < valData.length - 1; i++) {
   ...
   // alert(dept);
    ...
    if (acc == '0') {
        ...
        if ((complaint == "-1") || (complaint == "Yes" && compName == "" && compref == "" && compPrePos == "" && compDt == "" && compDtSub == "") || (complaint == "No" && compRej == "")) {
           ...
           // alert("Select Suggestions for consideration of RBI in FMR1-B");
            return;
        }
        ...
        if (FraudTypetxt == "GOLD") {
            ...
            var data1 = user + '~' + FraudID + '~' + FraudType + '~' + FraudNo + '~' + NBFCName + '~' + GoldFrType + '~' + GoldBrId + '~' + GoldBr + '~' + GoldPlace + '~' + GoldDis + '~' +
                ...
                commitTxt + '~' + calAmt + '~' + EntAmt + '~' + TotAmt + '~' + compRej;
           
        }
        else {

            var data1 = user + '~' + FraudID + '~' + FraudType + '~' + FraudNo + '~' + NBFCName + '~' + VehFrType + '~' + "" + '~' + VehBr + '~' + VehPlace + '~' + VehDis + '~' +
                VehState + '~' + PrinAcc + '~' + PrinAcctxt + '~' + ArOp + '~' + FrOcc + '~' + NatFr + '~' + commitdl + '~' + commitTxt + '~' + "" + '~' + EntAmt + '~' + TotAmt + '~' + compRej;
        }
        ...
        if (dept == '4' || dept == '23') {
            ...
            var data = user + '~' + FraudID + '~' + Dtoccto + '~' + DtoccFrm + '~' + DtDetTo + '~' + DtDetFrm + '~' + RsDelay
            ...
            var data = user + '~' + FraudID + '~' + claim + '~' + claimreason + '~' + Amt_recoverd + '~' + Amt_Insurance + '~' + Amt_Source
                + '~' + Total_amt;
    ...
    Swal.fire({
        ...
        if (result.value == true) {
            $.ajax({
                type: "post",
                contentType: "application/json; charset=utf-8",
                url: "FMR1.aspx/SaveFMR1",
                data: "{typ:'" + access + "', access :'" + access + "', val1 :'" + data + "', subdata :'" + loopdata + "', mdata :'" + FraudID + "'}",
                dataType: "json",
                success: function (Result) {
                    Result = Result.d;
                    if (acc == '0') {
    ...
    //    contentType: "application/json; charset=utf-8",
    //    url: "FMR1.aspx/SaveFMR1",
    //    data: "{typ:'" + access + "', access :'" + access + "', val1 :'" + data + "', subdata :'" + loopdata + "'}",
    //    dataType: "json",
    //    success: function (Result) {
    //        Result = Result.d;
